...
def taskAssignment(k, tasks):
	results = []
	sortedTasks = get_sorted_tasks_with_indices(tasks)
	
	idx1, idx2 = 0, len(tasks) - 1
	while len(results) < k:
		results.append([sortedTasks[idx1][0], sortedTasks[idx2][0]])
		idx1 += 1
		idx2 -= 1
		
	return results

def get_sorted_tasks_with_indices(tasks):
	tasks_indices = []
	for idx, t in enumerate(tasks):
		tasks_indices.append( (idx, t) )
	
	tasks_indices.sort(key=lambda x:x[1])
