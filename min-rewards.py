"""
    Imagine that you're a teacher who's just graded the final exam in a class. You
    have a list of student scores on the final exam in a particular order (not
    necessarily sorted), and you want to reward your students. You decide to do so
    fairly by giving them arbitrary rewards following two rules:

    All students must receive at least one reward.

    Any given student must receive strictly more rewards than an adjacent
    student (a student immediately to the left or to the right) with a lower
    score and must receive strictly fewer rewards than an adjacent student with
    a higher score.

    Write a function that takes in a list of scores and returns the minimum number
    of rewards that you must give out to students to satisfy the two rules.

    You can assume that all students have different scores; in other words, the
    scores are all unique.

    Example:
        scores = [8, 4, 2, 1, 3, 6, 7, 9, 5]

        Output:
            25 // you would give out the following rewards: [4, 3, 2, 1, 2, 3, 4, 5, 1]
"""

def minRewards(scores):
    rewards = [1] * len(scores)
    cycle_complete = False
    start_idx = scores.index(min(scores))
    idx = start_idx

    while not cycle_complete:
        if not get_is_valid(idx, scores, rewards):
            if idx > 0:
                if scores[idx - 1] < scores[idx]:
                    if rewards[idx - 1] >= rewards[idx]:
                        rewards[idx] = rewards[idx-1] + 1

            if idx < len(scores) - 1:
                if scores[idx + 1] < scores[idx] and not get_is_valid(idx, scores, rewards):
                    rewards[idx] = rewards[idx] + 1

        if idx < len(scores) - 1:
            idx += 1
        else:
            idx = 0

        if idx == start_idx:
            cycle_complete = True            

        # Go through again if we're not in a valid state
        if not get_is_valid(idx, scores, rewards):
            cycle_complete = False
            start_idx = idx

    return sum(rewards)

def get_is_valid(idx, scores, rewards):
    last_valid = idx == 0 or (scores[idx - 1] < scores[idx] and rewards[idx-1] < rewards[idx]) or (scores[idx - 1] > scores[idx] and rewards[idx-1] > rewards[idx])
    next_valid = idx == len(scores) - 1 or (scores[idx + 1] < scores[idx] and rewards[idx+1] < rewards[idx]) or (scores[idx + 1] > scores[idx] and rewards[idx+1] > rewards[idx])

    return last_valid and next_valid


assert minRewards([8, 4, 2, 1, 3, 6, 7, 9, 5]) == 25 # [4, 3, 2, 1, 2, 3, 4, 5, 1]
assert minRewards([5, 10]) == 3